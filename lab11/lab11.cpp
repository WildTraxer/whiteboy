...
void _tmain ()
{
	...
	ofstream file;
	//создаем объект Readfile для чтения из текстового файла
	ifstream read_file;
	...
	switch (CmdMenu)
	{
        ...
        case DELETE_ELEM:
            ...
            cout << "Список с удаленным элементом" << endl;
		...
		case SAVE_FILE:
			...
			file.close();
		break;
		// чтение элементов списка из файлп
		case READ_FILE:
			cout << "Введите файл . txt для чтения списка >> ";
			cin >> NameFileTxt;
			...
			char *mass;
			int SizeFile; // Для размера файла
			read_file.seekg (0, ios::end); // Ставим курсор в начало файла
			SizeFile = read_file.tellg(); // Определение размера файла
			read_file.seekg(0, ios::beg); // Возвращаем курсор в начало файла.
			mass = new char [SizeFile]; // Динамическе выделение памяти для массива list
			int SizeProgram=0;
			while(!read_file.eof())
			...
			cout<<endl;
			// Устанавливаем NULL в конце строки
	...
	int Menu ()
	{
		int CmdMenu;
		 ...
		 for (;;)
		 {
			 ...
			 cout << DELETE_ELEM << " - удалить элемент" << endl;
			 cout << DELETE_ALL << " - удалить весь список" << endl;
			 ...
			 cout << READ_FILE << " - чтение из файла" << endl;
